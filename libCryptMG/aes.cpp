...
int main()
{
    const ::std::uint8_t test_key[AES256_NUM_KEYWORDS * sizeof(::std::uint32_t)] =
    { 0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe, 0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
    0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61, 0x08, 0xd7, 0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4 };

    const ::std::uint32_t test_result[8] =
    { 0x603deb10, 0x15ca71be, 0x2b73aef0, 0x857d7781, 0x1f352c07,
    0x3b6108d7, 0x2d9810a3, 0x0914dff4 };
    ::libcryptmg::core::cipher_key * base_key = ::libcryptmg::crypto::generate_aes_key(test_key, sizeof(test_key));

    aes_key * result_key = dynamic_cast<aes_key*>(base_key);
    if(!result_key)
    {
        std::cout<< "generate returned the wrong object" << std::endl;
        return -1;
    }
    int result = std::memcmp(&test_result[0], &(result_key->_key_vec[0]), result_key->size());

    if (!result)
        ::std::cout << "Key expansion 256 validated" <<::std::endl;
    else
        ::std::cout << "Key expansion 256 failed" <<::std::endl;
